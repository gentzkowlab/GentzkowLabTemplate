...
run_shell forever.sh "${LOGFILE}" || exit 1
run_python forever.py "${LOGFILE}" || exit 1
run_R forever.r "${LOGFILE}" || exit 1
run_julia forever.jl "${LOGFILE}" || exit 1
run_matlab forever.m "${LOGFILE}" || exit 1
run_stata forever.do "${LOGFILE}" || exit 1
run_notebook forever.ipynb "${LOGFILE}" || exit 1
run_rmd forever.Rmd "${LOGFILE}" || exit 1
run_latex forever.tex "${LOGFILE}" || exit 1
run_lyx forever.lyx "${LOGFILE}" || exit 1
) || false
